...
# ✅ FIX 1: Default Rclone path ab 'mega:' set kar diya gaya hai
RCLONE_PATH = os.environ.get("RCLONE_PATH", "mega:")
...
def extract_archive(file_path):
    ...
    if not shutil.which("7z"): return [], None, "7z not installed on server!"
...
async def rclone_upload_file(message, file_path, queue_pos=None):
    ...
    await process.wait()
    # ✅ FIX: Yahan se success text hata diya taaki next file aane par bar gayab na ho
...
async def upload_file(client, message, file_path, user_mention, queue_pos=None):
    try:
        ...
        try:
            ...
            # ✅ FIX 2: Yahan se "Uploaded to Dump" wala link message edit hataya gaya hai.
            # Ab progress bar overwrite nahi hoga aur smooth chalega!
