...
async def web_server():
    async def handle(request): return web.Response(text="Cloud Bot Running!")
    app = web.Application()
...
async def set_dump(c, m):
    try:
        ...
        save_config()
        await m.reply_text(f"✅ Dump Chat ID Set: `{chat_id}`")
...
async def auth(c, m):
    try:
        ...
        if uid not in config["auth_users"]:
            ...
            save_config()
            await m.reply_text(f"✅ Authorized: `{uid}`")
    except: pass
