    ...
    try:
                ...
                async with sess.get(link) as res:
                    if res.status == 200:
                        ...
                        with open(meta_path, "wb") as f:
                            f.write(data)
                        # Ab is file ko Aria2 mein load karo content download karne ke liye
                        download = aria2.add_torrent(meta_path)
        
        # Scenario B: Magnet Link
        elif link.startswith("magnet:"):
            download = aria2.add_magnet(link)
        
        # Scenario C: Other HTTP (should allow Aria to decide)
        else:
        ...
        while True:
            ...
            if download.status == "complete":
                # Download complete, find the main video file
        ...
        if file_path and os.path.exists(file_path):
            ...
            if DUMP_ID != 0:
                try:
                    ...
                    await message.reply_text(f"⚠️ **Dump Error:** Bot channel mein Admin nahi hai ya ID galat hai.\nError: `{e}`")
